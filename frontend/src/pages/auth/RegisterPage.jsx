...
export default function RegisterPage() {
      ...
      <div className="relative">
        <input
          ...
          className={`input ${errors[name] ? 'input-error' : ''} ${
            name === 'password_confirm' || name === 'password' ? 'pr-12' : ''
          }`}
        />
        {(name === 'password') && (
  ...
  return (
    <div className="min-h-screen bg-surface-50 flex">
      {/* Left — branding */}
      <div className="hidden lg:flex lg:w-5/12 bg-gradient-to-br from-brand-950 via-brand-900 to-indigo-900
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2.5 mb-16">
            ...
            <span className="font-display font-800 text-white text-xl">QuizAI</span>
          </div>

          <h2 className="font-display text-3xl font-800 text-white leading-tight mb-3">
            Your smartest<br />study partner
          </h2>
          ...
          <div className="space-y-3">
            {PERKS.map((p) => (
              <div key={p} className="flex items-center gap-3">
                ...
                <span className="text-brand-100 text-sm">{p}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-10 overflow-y-auto">
        <div className="w-full max-w-lg animate-slide-up">
          <div className="flex items-center gap-2 mb-6 lg:hidden">
            ...
            <span className="font-display font-700 text-slate-900">QuizAI</span>
          </div>

          <h1 className="font-display text-2xl font-700 text-slate-900 mb-1">Create your account</h1>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-2 gap-4">
              <Field name="first_name" label="First name" placeholder="Alex" half />
              <Field name="last_name"  label="Last name"  placeholder="Kim" half />
              <Field name="username" label="Username" placeholder="alexkim" />
              <Field name="email" label="Email address" type="email" placeholder="alex@example.com" />
